//! Background sync worker that periodically uploads messages to File Storage.
...
/// Prefix for message patch identifiers in File Storage quilts.
/// Must match MSG_PREFIX in file-storage-discovery-indexer/src/constants.ts
const MSG_PREFIX: &str = "msg-";

/// Source tag value to identify patches from this relayer.
/// Must match SOURCE_TAG in file-storage-discovery-indexer/src/constants.ts
const SOURCE_TAG: &str = "myso-messaging-relayer";

/// Background service that syncs pending messages to File Storage storage.
pub struct FileStorageSyncService {
    /// Storage backend to query for pending messages
    storage: Arc<dyn StorageAdapter>,
    /// File Storage HTTP client for uploading quilts
    file_storage_client: Arc<FileStorageClient>,
    /// How often to run the sync cycle (seconds)
    sync_interval_secs: u64,
    /// Max messages to batch per sync cycle
    batch_size: usize,
    /// Number of File Storage epochs to store each quilt
    storage_epochs: u32,
...
impl FileStorageSyncService {
    pub fn new(
        ...
        Self {
            ...
            batch_size: config.file_storage_sync_batch_size.min(666),
            storage_epochs: config.file_storage_storage_epochs,
            sync_rx,
    ...
    pub async fn run(&mut self) {
        info!(
            "Starting FileStorageSyncService (interval={}s, batch_size={}, epochs={}, message_threshold={})",
        ...
        loop {
            ...
            tokio::select! {
                _ = ticker.tick() => {
                    debug!("File Storage sync triggered by timer (message_count was {})", message_count);
                }
                ...
                result = self.sync_rx.recv() => {
                    match result {
                        Some(()) => {
                            ...
                            debug!(
                                "File Storage sync triggered by message threshold ({}/{})",
                                message_count, self.message_threshold
                            );
                        }
                        None => {
                            warn!("Sync notification channel closed, stopping FileStorageSyncService");
                            return;
                        }
                    }
                }
            ...
            if let Err(e) = self.sync_pending_messages().await {
                error!("File Storage sync cycle failed (SyncPending): {}", e);
            }
            if let Err(e) = self.sync_updated_messages().await {
                error!("File Storage sync cycle failed (UpdatePending): {}", e);
            }
            if let Err(e) = self.sync_deleted_messages().await {
                error!("File Storage sync cycle failed (DeletePending): {}", e);
            }
    ...
    /// File Storage but is superseded by the new one.
    ...
    async fn sync_messages(
        ...
        info!("Syncing {} {} messages to FileStorage", messages.len(), label);

        // 2. Build patches and per-patch metadata tags.
        // Serialize with the target status so the File Storage copy reflects the final state.
        ...
        for msg in &messages {
            ...
            let mut msg_for_file_storage = msg.clone();
            msg_for_file_storage.sync_status = to_status;

            match serde_json::to_vec(&msg_for_file_storage) {
                Ok(data) => {
        ...
        // 3. Upload the batch as a single quilt to File Storage with per-patch tags
        let response = self
            .file_storage_client
        ...
        info!(
            "Quilt stored on File Storage ({}). Blob ID: {}, patches: {}",
            label,
        ...
        for msg in &messages {
            ...
            match response.get_patch_id(&identifier) {
                ...
                None => {
                    warn!(
                        "No patch ID found in File Storage response for {} message {}",
                        label, msg.id
                    );
                }
            }
        ...
        info!("File Storage {} sync cycle completed successfully", label);
        Ok(())
    }
}
