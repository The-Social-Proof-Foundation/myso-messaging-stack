//! File Storage HTTP client for storing and retrieving blobs/quilts.
...
use super::types::{
    BlobStoreResponse, PatchInfo, QuiltPatchMetadata, QuiltStoreResponse, FileStorageError, FileStorageResult,
};

/// HTTP client for the File Storage publisher and aggregator APIs
#[derive(Debug, Clone)]
...
impl FileStorageClient {
    /// Creates a new FileStorageClient with the given publisher and aggregator URLs
    ...
    /// * `epochs` - Number of File Storage epochs to store the quilt
    pub async fn store_quilt(
        &self,
        patches: Vec<(impl Into<String>, Vec<u8>)>,
        metadata: Option<Vec<QuiltPatchMetadata>>,
        epochs: u32,
    ) -> FileStorageResult<QuiltStoreResponse> {
        if patches.is_empty() {
            return Err(FileStorageError::ApiError {
                status: 400,
        ...
        if let Some(meta) = metadata {
            let meta_json =
                serde_json::to_string(&meta).map_err(|e| FileStorageError::ParseError(e.to_string()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(FileStorageError::RequestFailed)?;

        // Check status and parse response
        let status = response.status();
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            error!("File Storage store_quilt failed: {} - {}", status.as_u16(), body);
            return Err(FileStorageError::ApiError {
        ...
        let result: QuiltStoreResponse = response
            .json()
            .await
            .map_err(|e| FileStorageError::ParseError(e.to_string()))?;
    ...
    /// * `epochs` - Number of File Storage epochs to store the blob
    ///
    /// # Returns
    ...
    #[allow(dead_code)]
    pub async fn store_blob(&self, data: Vec<u8>, epochs: u32) -> FileStorageResult<BlobStoreResponse> {
        ...
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            error!("File Storage store_blob failed: {} - {}", status.as_u16(), body);
            return Err(FileStorageError::ApiError {
                status: status.as_u16(),
                message: body,
            });
    ...
    #[allow(dead_code)]
    pub async fn read_by_patch_id(&self, patch_id: &str) -> FileStorageResult<Vec<u8>> {
        let url = format!(
        ...
        if status == reqwest::StatusCode::NOT_FOUND {
            error!("File Storage read_by_patch_id not found: {}", patch_id);
            return Err(FileStorageError::NotFound(patch_id.to_string()));
        }
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            error!(
                "File Storage read_by_patch_id failed: {} - {}",
                status.as_u16(),
                body
            );
            return Err(FileStorageError::ApiError {
        ...
        let bytes = response.bytes().await.map_err(FileStorageError::RequestFailed)?;
    ...
    #[allow(dead_code)]
    pub async fn read_blob(&self, blob_id: &str) -> FileStorageResult<Vec<u8>> {
        ...
        let status = response.status();
        if status == reqwest::StatusCode::NOT_FOUND {
            error!("File Storage read_blob not found: {}", blob_id);
            return Err(FileStorageError::NotFound(blob_id.to_string()));
        }
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            error!("File Storage read_blob failed: {} - {}", status.as_u16(), body);
            return Err(FileStorageError::ApiError {
                status: status.as_u16(),
                message: body,
            });
    ...
    #[allow(dead_code)]
    pub async fn list_patches(&self, quilt_blob_id: &str) -> FileStorageResult<Vec<PatchInfo>> {
        let url = format!(
            "{}/v1/quilts/{}/patches",
        ...
        let status = response.status();
        if status == reqwest::StatusCode::NOT_FOUND {
            error!("File Storage list_patches not found: {}", quilt_blob_id);
            return Err(FileStorageError::NotFound(quilt_blob_id.to_string()));
        }
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            error!("File Storage list_patches failed: {} - {}", status.as_u16(), body);
            return Err(FileStorageError::ApiError {
                status: status.as_u16(),
                message: body,
            });
