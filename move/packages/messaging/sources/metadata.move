...
/// Returns the value for `key` when present in the data map.
public(package) fun get_data_value(self: &Metadata, key: &String): Option<String> {
    if (self.data.contains(key)) {
        option::some(*self.data.get(key))
    } else {
        option::none()
    }
}
