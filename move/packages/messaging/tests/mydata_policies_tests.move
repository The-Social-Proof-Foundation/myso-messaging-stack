#[test_only]
module messaging::mydata_policies_tests;
...
/// Builds a valid MyData identity bytes.
...
// === mydata_approve_reader tests ===

#[test]
fun mydata_approve_reader_valid_identity_and_permission() {
    ...
    mydata_policies::mydata_approve_reader(id, &version, &group, &encryption_history, ts.ctx());
...
#[test]
fun mydata_approve_reader_with_rotated_key_version() {
    let mut ts = ts::begin(ALICE);
    let group_id = setup_group(&mut ts);
    ...
    mydata_policies::mydata_approve_reader(build_identity(group_id, 0), &version, &group, &encryption_history, ts.ctx());
    mydata_policies::mydata_approve_reader(build_identity(group_id, 1), &version, &group, &encryption_history, ts.ctx());
...
#[test]
fun mydata_approve_reader_member_with_reader_permission() {
    let mut ts = ts::begin(ALICE);
    let group_id = setup_group(&mut ts);
...
#[test, expected_failure(abort_code = mydata_policies::EInvalidIdentity)]
fun mydata_approve_reader_invalid_group_id_fails() {
    let mut ts = ts::begin(ALICE);
    setup_group(&mut ts);
...
#[test, expected_failure(abort_code = mydata_policies::EInvalidIdentity)]
fun mydata_approve_reader_short_id_fails() {
    let mut ts = ts::begin(ALICE);
    setup_group(&mut ts);
    ...
    mydata_policies::mydata_approve_reader(short_id, &version, &group, &encryption_history, ts.ctx());

    abort
}

#[test, expected_failure(abort_code = mydata_policies::EInvalidKeyVersion)]
fun mydata_approve_reader_future_key_version_fails() {
    let mut ts = ts::begin(ALICE);
    let group_id = setup_group(&mut ts);
...
#[test, expected_failure(abort_code = mydata_policies::ENotPermitted)]
fun mydata_approve_reader_without_permission_fails() {
    let mut ts = ts::begin(ALICE);
    let group_id = setup_group(&mut ts);
...
#[test, expected_failure(abort_code = mydata_policies::ENotPermitted)]
fun mydata_approve_reader_non_member_fails() {
    let mut ts = ts::begin(ALICE);
    let group_id = setup_group(&mut ts);
...
#[test, expected_failure(abort_code = mydata_policies::EEncryptionHistoryMismatch)]
fun mydata_approve_reader_mismatched_encryption_history_fails() {
    let mut ts = ts::begin(ALICE);
    ...
    mydata_policies::mydata_approve_reader(id, &version, &group1, &encryption_history2, ts.ctx());
