...
#[test, expected_failure(abort_code = msg::EEncryptionHistoryMismatch)]
...
fun leave_removes_member() {
    ...
    msg::leave(&group_leaver, &mut group, ts.ctx());
...
#[test, expected_failure(abort_code = msg::EPermissionsAdminCannotLeave)]
...
#[test, expected_failure(abort_code = myso::permissioned_group::EMemberNotFound)]
...
fun set_group_name_succeeds_with_metadata_admin() {
    ...
    msg::set_group_name(&group_manager, &mut group, new_name, ts.ctx());
...
fun insert_and_remove_group_data() {
    ...
    msg::insert_group_data(&group_manager, &mut group, key, value, ts.ctx());
    ...
    // Remove the data
    let (removed_key, removed_value) = msg::remove_group_data(
        &group_manager,
        &mut group,
        &key,
        ts.ctx(),
...
fun archive_group(ts: &mut ts::Scenario) {
    ...
    let version = ts.take_shared<Version>();
    let mut group = ts.take_shared<PermissionedGroup<Messaging>>();
    msg::archive_group(&version, &mut group, ts.ctx());
    ts::return_shared(group);
    ts::return_shared(version);
}

#[test, expected_failure(abort_code = msg::EGroupArchived)]
