#[test_only]
module myso_messaging::group_handle_registry_tests;

use myso_messaging::group_handle_registry::GroupHandleRegistry;
...
fun set_group_handle_and_lookup_succeeds() {
    ...
    let found = messaging::lookup_group_by_handle(&registry, string::utf8(b"aliceteam"));
...
#[test, expected_failure(abort_code = myso_messaging::group_handle_registry::EHandleTaken)]
fun duplicate_group_handle_fails() {
    ...
    let (g1, eh1, ml1) = messaging::create_group(
        &version,
        &mut namespace,
        &group_manager,
        string::utf8(TEST_GROUP_NAME),
    ...
    let (g2, eh2, ml2) = messaging::create_group(
        &version,
        &mut namespace,
        &group_manager,
        string::utf8(TEST_GROUP_NAME),
    ...
    messaging::set_group_handle(&version, &mut registry, &mut g1, h, ts.ctx());
    messaging::set_group_handle(
        &version,
        &mut registry,
        &mut g2,
